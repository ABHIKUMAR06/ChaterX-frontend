...
export default function ChatWindow({ chatId, name, onMessageSent, socket }: ExtendedChatWindowProps) {
  ...
  useEffect(() => {
    ...
    const handleStatusUpdate = ({ messageId, status, chatId: updateChatId }: {
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg._id === messageId ? { ...msg, status: { ...msg.status, name: status } } : msg
        )
      );
    };
  ...
  return (
    <div ref={containerRef} className="flex flex-col h-full w-full max-w-full bg-orange-50">
      ...
      {replyTo && (
        <div className="p-2 bg-orange-100 border-l-4 border-orange-400 text-sm mb-1 flex justify-between items-center rounded">
          <div>
      ...
      <div className="p-3 bg-white flex items-center gap-2 shadow-lg border-t border-gray-100">
        <textarea
